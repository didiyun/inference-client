...
from tf_utils import tensor_shape
from tf_utils import dtypes

# Fallback in case fast_tensor_util is not properly compiled.
# pylint: disable=g-import-not-at-top
try:
  import fast_tensor_util
...
def GetFromNumpyDTypeDict(dtype_dict, dtype):
  ...
  for key, val in _six.iteritems(dtype_dict):
    if key == dtype:
      return val
  return None
...
def _FilterNotTensor(v):
  ...
  return str(v) if isinstance(v, Tensor) else None
...
def make_tensor_proto(values, dtype=None, shape=None, verify_shape=False,
  ...
  if numpy_dtype == dtypes.string and not isinstance(values, np.ndarray):
    ...
    try:
      str_values = [as_bytes(x) for x in proto_values]
...
def is_tensor(x):  # pylint: disable=invalid-name
  ...
  """
  return (isinstance(x, _TensorLike) or isinstance(x, tuple()) or  # pylint: disable=protected-access
          isinstance(x, CompositeTensor) or
